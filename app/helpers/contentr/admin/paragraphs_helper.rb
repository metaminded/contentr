      ...
      def display_paragraph(p, current = true)
        ...
        when Contentr::HtmlParagraph
          if contentr_publisher? && current && p.unpublished_changes?
            ...
            s += image_tag(p.image_unpublished_url) if p.unpublished_data.has_key?("image_unpublished") && p.image_unpublished.present? && !p.remove_image?
          else
            s = p.data[:body].html_safe if p.data.has_key?(:body)
            s += image_tag(p.image_url) if p.data.has_key?("image") && p.image?
          end
        end
        s
      end
