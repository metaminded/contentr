# encoding: utf-8
class Contentr::Admin::ContentrParagraphsController < Contentr::Admin::ApplicationController
  ...
  def edit
    @page = Contentr::Page.find(params[:contentr_page_id])
  ...
  def update
    @page = Contentr::Page.find(params[:contentr_page_id])
    @paragraph = @page.paragraphs.select { |p| p.id.to_s == params[:id] }.first
    if @paragraph.update_attributes(params[:contentr_paragraph])
      flash[:notice] = 'Erfolgreich geändert'
      redirect_to edit_contentr_admin_contentr_page_contentr_paragraph_path(@page, @paragraph)
    else
      render :action => :edit
    end
  end
